...
function getWeather(lon, lat) {
  api
    .get(
      `weather?lat=${lat}&lon=${lon}&lang=${apiOptions.lang}&units=${apiOptions.units}&appid=${apiOptions.key}`
    )
    .then((res) => {
      if (!res.status == 200) {
        alert("falha");
...
function searchCityClimate() {
  ...
  api
    .get(
      `weather?q=${input}&lang=${apiOptions.lang}&units=${apiOptions.units}&appid=${apiOptions.key}`
    )
    .then((res) => {
      if (!res.status == 200) {
        alert("falha");
